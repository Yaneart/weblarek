...
`category` --- категория товара (автоматически применяет CSS-класс)
...
`buttonText` --- текст кнопки ("В корзину"/"В корзине")

`buttonDisabled` --- состояние кнопки

Особенности: Кнопка меняет состояние в зависимости от наличия товара в корзине.
...
`list` --- массив элементов CardBasket

`total` --- общая стоимость заказа

`buttonDisabled` --- состояние кнопки оформления
...
`buttonDisabled ` --- состояние кнопки продолжения

Методы класса ModalOrder :

`get formData(): IOrderForm` - возвращает данные формы
...
`buttonDisabled` --- состояние кнопки отправки

Методы класса ModalContacts :

`get formData(): IContactsForm` - возвращает данные формы
...
`counter` --- счетчик товаров в корзине (отображается в хедере)

`catalog` --- массив карточек товаров для отображения в галерее

`locked `--- блокировка интерфейса (при открытых модальных окнах)
